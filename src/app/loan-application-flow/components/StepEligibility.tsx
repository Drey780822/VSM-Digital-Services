...
export default function StepEligibility({ data, updateData, onNext, onBack }: Props) {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<EligibilityForm>({
    defaultValues: {
  ...
  const onCheck = (values: EligibilityForm) => {
    ...
    updateData({ salary: Number(values.salary), expenses: Number(values.expenses), qualifiedAmount: qualified, purpose: values.purpose });
  };

  const monthly = (() => {
    if (!selectedAmount || !selectedTerm) return 0;
    const r = INTEREST_RATE / 12;
    return (selectedAmount * r * Math.pow(1 + r, selectedTerm)) / (Math.pow(1 + r, selectedTerm) - 1);
  })();
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-7 lg:p-8">
      <div className="mb-6">
        <h2 className="font-display text-2xl font-semibold text-foreground mb-1">Check Your Eligibility</h2>
        <p className="text-sm text-foreground-muted">Enter your monthly income and expenses to get an instant estimate.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onCheck)} className="space-y-5 mb-6">
        <div>
          ...
          <p className="text-xs text-foreground-muted mb-1.5">Your total income before deductions</p>
          <div className="relative">
            <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-foreground-muted font-medium text-sm">R</span>
            <input
              {...register('salary', {
        ...
        <div>
          ...
          <p className="text-xs text-foreground-muted mb-1.5">Rent, food, transport, existing loan repayments</p>
          <div className="relative">
            <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-foreground-muted font-medium text-sm">R</span>
            <input
              {...register('expenses', {
          ...
          {errors.expenses && <p className="text-xs text-danger mt-1.5">{errors.expenses.message}</p>}
          {salary > 0 && expenses > 0 && (
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className={`text-xs font-medium ${dtiRatio > 70 ? 'text-danger' : dtiRatio > 50 ? 'text-warning' : 'text-success'}`}>
        ...
        <div>
          ...
          >
            ...
            {LOAN_PURPOSES.map((p) => (
              <option key={`purpose-${p}`} value={p}>{p}</option>
            ))}
          </select>
      ...
      {/* Result */}
      {result !== null && (
        <div className={`rounded-xl border p-5 mb-6 ${result > 0 ? 'border-success/40 bg-success/5' : 'border-danger/40 bg-danger/5'}`}>
          {result > 0 ? (
            <>
              ...
              <p className="text-xs text-foreground-muted mb-5">Maximum pre-approved amount · Subject to final review</p>

              {/* Amount Slider */}
              <div className="mb-4">
                <div className="flex justify-between text-xs text-foreground-muted mb-1.5">
                  <span>Loan Amount</span>
                  <span className="text-primary font-semibold counter-value">R {selectedAmount.toLocaleString()}</span>
                </div>
                <input
                  type="range"
                  min={1000}
                  ...
                  {TERMS.map((t) => (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-md text-xs font-medium transition-all duration-200 ${
                        selectedTerm === t ? 'bg-gold-gradient text-background font-semibold' : 'bg-muted text-foreground-muted hover:text-foreground border border-border'
                      }`}
                    >
                      {t}m
                    </button>
              ...
              <div className="bg-background rounded-lg p-4 flex items-center justify-between">
                <div>
                  ...
                  <div className="font-display text-2xl font-semibold text-gradient-gold counter-value">R {Math.ceil(monthly).toLocaleString()}</div>
                </div>
                <div className="text-right">
                  <div className="text-xs text-foreground-muted">Total Repayable</div>
                  <div className="text-sm font-semibold text-foreground counter-value">R {Math.ceil(monthly * selectedTerm).toLocaleString()}</div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2 mt-3 p-3 bg-info/5 border border-info/20 rounded-lg">
                ...
                <p className="text-[10px] text-foreground-muted leading-relaxed">
                  This is a pre-qualification estimate at 18% p.a. Final approval is subject to credit checks and VSCOBAR&apos;s lending criteria.
                </p>
              </div>
            </>
          ) : (
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-foreground-muted">
                  Based on your current income and expenses, we&apos;re unable to offer a pre-approval. Please reduce your expenses or contact us to explore options.
                </p>
              </div>
            </div>
          )}
