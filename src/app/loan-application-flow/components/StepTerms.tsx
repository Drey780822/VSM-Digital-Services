    ...
    <div className="bg-card border border-border rounded-2xl p-7 lg:p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-foreground-muted">
          Review your loan summary and authorise the debit order before submitting.
        </p>
      </div>
      ...
      <div className="bg-muted/40 border border-border rounded-xl p-5 mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <div key={`summary-${item.label}`}>
              <div className="text-[10px] font-semibold text-foreground-muted tracking-wide uppercase mb-0.5">
                {item.label}
              </div>
        ...
        <div className="mt-4 p-3 bg-info/5 border border-info/20 rounded-lg flex items-start gap-2">
          ...
          <p className="text-[10px] text-foreground-muted leading-relaxed">
            A 5% late payment penalty applies to overdue instalments. Debit order runs on the 1st of
            each month. Missed payments are reported to the NCR.
          </p>
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="bg-muted/30 border border-border rounded-xl p-4 h-40 overflow-y-auto text-xs text-foreground-muted leading-relaxed mb-3 space-y-2">
          <p>
            <strong className="text-foreground">1. Loan Agreement.</strong> This agreement is
            between the Applicant and VSM Financial Services (Pty) Ltd, a registered credit provider
            under the National Credit Act (NCA) of South Africa.
          </p>
          <p>
            <strong className="text-foreground">2. Interest Rate.</strong> The annual interest rate
            is 18% per annum, calculated monthly on the reducing balance. The initiation fee is
            included in the total cost of credit disclosed.
          </p>
          <p>
            <strong className="text-foreground">3. Repayment.</strong> Monthly instalments are due
            on the 1st of each calendar month via debit order. The applicant authorises VSM to debit
            the stated bank account for the agreed instalment amount.
          </p>
          <p>
            <strong className="text-foreground">4. Late Payment.</strong> A penalty of 5% of the
            overdue amount will be charged for any instalment not received within 5 business days of
            the due date. Persistent non-payment may result in legal action and credit bureau
            reporting.
          </p>
          <p>
            <strong className="text-foreground">5. Privacy.</strong> Your personal information is
            processed in accordance with the Protection of Personal Information Act (POPIA). We will
            not sell or share your data with third parties without your consent.
          </p>
          <p>
            <strong className="text-foreground">6. Cancellation.</strong> You may cancel this
            agreement within 5 business days of signing without penalty. After this cooling-off
            period, early settlement is allowed with a settlement fee equivalent to one month&apos;s
            instalment.
          </p>
        </div>
        <label className="flex items-start gap-3 cursor-pointer group">
          <div
            ...
            className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-all duration-200 cursor-pointer ${
              termsAccepted
                ? 'bg-gold-gradient border-primary'
          ...
          <span className="text-xs text-foreground-muted leading-relaxed">
            ...
            <span className="text-primary">Terms and Conditions</span> and the{' '}
            <span className="text-primary">National Credit Act</span> disclosures above.{' '}
      ...
      <div className="mb-6">
        ...
        <div className="bg-muted/30 border border-border rounded-xl p-4 mb-3">
          <p className="text-xs text-foreground-muted leading-relaxed">
            ...
            , hereby authorise VSM Financial Services (Pty) Ltd to debit my{' '}
            <span className="text-foreground font-semibold">
              {data.bankName || 'selected bank'}
            </span>{' '}
            account <span className="text-foreground font-mono">{data.accountNumber || '—'}</span>{' '}
            with{' '}
            <span className="text-primary font-semibold">
              R {Math.ceil(monthly).toLocaleString()}
            </span>{' '}
            on the 1st of each month for{' '}
            <span className="text-primary font-semibold">{data.term} months</span>, commencing on
            the 1st of the month following loan approval.
          </p>
        </div>
        <label className="flex items-start gap-3 cursor-pointer group">
          <div
          ...
          <span className="text-xs text-foreground-muted leading-relaxed">
            I authorise the debit order as described above and confirm that I am the account holder
            of the stated bank account. <span className="text-danger">*</span>
          </span>
        </label>
      </div>
